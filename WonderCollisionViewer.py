...
def create_capsule(capsules_data):
    ...
    for capsule_data in capsules_data:
        capsule = {
            "CenterA": (parse_number(capsule_data["CenterA"]["X"]), parse_number(capsule_data["CenterA"]["Y"])),
            "CenterB": (parse_number(capsule_data["CenterB"]["X"]), parse_number(capsule_data["CenterB"]["Y"])),
            "OffsetRotation": (0, 0, parse_number(capsule_data["OffsetRotation"]["Z"])),
            "OffsetTranslation": (parse_number(capsule_data["OffsetTranslation"]["X"]), parse_number(capsule_data["OffsetTranslation"]["Y"])),
            "Radius": parse_number(capsule_data["Radius"]),
        }
...
def create_box(boxes_data):
    ...
    for box_data in boxes_data:
        center_x, center_y = parse_number(box_data["Center"]["X"]), parse_number(box_data["Center"]["Y"])
        half_extents_x = parse_number(box_data["HalfExtents"]["X"]) if "HalfExtents" in box_data else 1.0
        half_extents_y = parse_number(box_data["HalfExtents"]["Y"]) if "HalfExtents" in box_data else 1.0
        rotation = (0, 0, parse_number(box_data["OffsetRotation"]["Z"]) if "OffsetRotation" in box_data else 0)
        translation = (parse_number(box_data["OffsetTranslation"]["X"]) if "OffsetTranslation" in box_data else 0,
                       parse_number(box_data["OffsetTranslation"]["Y"]) if "OffsetTranslation" in box_data else 0)
...
def create_sphere(spheres_data):
    ...
    for sphere_data in spheres_data:
        sphere = {
            ...
            "Radius": parse_number(sphere_data["Radius"]),
            "OffsetTranslation": (parse_number(sphere_data["OffsetTranslation"]["X"]) if "OffsetTranslation" in sphere_data else 0,
                                  parse_number(sphere_data["OffsetTranslation"]["Y"]) if "OffsetTranslation" in sphere_data else 0)
        }
...
def create_cylinder(cylinders_data):
    ...
    for cylinder_data in cylinders_data:
        cylinder = {
            "CenterA": (parse_number(cylinder_data["CenterA"]["X"]), parse_number(cylinder_data["CenterA"]["Y"])),
            "CenterB": (parse_number(cylinder_data["CenterB"]["X"]), parse_number(cylinder_data["CenterB"]["Y"])),
            "OffsetRotation": (parse_number(cylinder_data["OffsetRotation"]["X"]), parse_number(cylinder_data["OffsetRotation"]["Y"]), parse_number(cylinder_data["OffsetRotation"]["Z"])),
            "OffsetTranslation": (parse_number(cylinder_data["OffsetTranslation"]["X"]), parse_number(cylinder_data["OffsetTranslation"]["Y"])),
            "Radius": parse_number(cylinder_data["Radius"]),
        }
...
def create_prism(prisms_data):
    ...
    for prism_data in prisms_data:
        prism = {
            "HeightHead": parse_number(prism_data["HeightHead"]),
            "HeightShoulder": parse_number(prism_data["HeightShoulder"]),
            "OffsetTranslation": (parse_number(prism_data["OffsetTranslation"]["X"]), parse_number(prism_data["OffsetTranslation"]["Y"])),
            "Radius": parse_number(prism_data["Radius"]),
        }
        prisms.append(prism)
...
def create_polytope(polytopes_data):
    ...
    for polytope_data in polytopes_data:
        vertices = [(parse_number(vertex["X"]), parse_number(vertex["Y"])) for vertex in polytope_data["Vertices"]]
        polytope = {
            "OffsetRotation": (parse_number(polytope_data["OffsetRotation"]["X"]), parse_number(polytope_data["OffsetRotation"]["Y"]), 0),  # Set Z to zero
            "OffsetTranslation": (parse_number(polytope_data["OffsetTranslation"]["X"]), parse_number(polytope_data["OffsetTranslation"]["Y"])),
            "Vertices": vertices,
        }
...
def plot_polytope(ax, polytope):
    ...
    # Convert the transformed vertices to a numpy array
    points = np.array(transformed_vertices)
    ...
    # Create a polygon with the hull vertices
    poly = patches.Polygon(hull_vertices, closed=True, linewidth=1, edgecolor='m', facecolor='m', alpha=0.5)
    ax.add_patch(poly)

    print(f"Hull vertices: {hull_vertices.tolist()}")
